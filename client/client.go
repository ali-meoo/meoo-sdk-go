 ...
 * 通过 Projects()/Agent() 调用；API() 返回覆盖全部 operation 的完整 API 客户端。Client
...
// API 返回与本客户端共享 baseURL 与 *http.Client 的完整 API 客户端，覆盖 Projects()/Agent()
...
// Context 在 ctx 上注入当前 Bearer 凭证，供 API() 返回的 API 客户端使用；凭证动态取值失败
// 返回 *TransportError。
...
// newAPIClient 用相同的 baseURL 与 *http.Client 构造底层 API 客户端：覆写 server 列表为
